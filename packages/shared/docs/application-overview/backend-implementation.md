...
**ID Handling Pattern** (`isTemporaryId`):
- **Purpose**: Distinguish between temporary frontend IDs and real database IDs
- **Architecture Decision**: Part of the "Always Send IDs Pattern" where frontend always sends IDs (temporary for new items, real for existing) and backend handles ID resolution
- **Temporary ID Format**: Generated by frontend using `Date.now() + Math.random()`, creating very large numbers (>= 1 billion) that may have decimal parts
- **Real Database ID Format**: Positive integers (no decimals), typically < 1 billion
- **Implementation**: `isTemporaryId()` checks if ID is >= 1 billion or has decimal parts
- **Benefits**: Eliminates frontend logic for distinguishing new vs existing items, backend handles all ID resolution

**Source File**: `apps/backend/src/features/class/classService.ts` (isTemporaryId function)

**Related Documentation**:
- [Frontend Patterns](frontend-patterns.md#always-send-ids-pattern) - Frontend implementation of Always Send IDs Pattern
