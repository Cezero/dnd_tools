...
Each of the monster entries describes a typical creature of its kind. However, there are several methods by which **extraordinary or unique monsters** can be created using a typical creature as the foundation:

- **Adding character classes**
- **Increasing a monster's Hit Dice** 
- **Adding a template to a monster**
...
> **See also**: [Monsters as Races](monsters-as-races.md)
...
> **Note**: If a creature acquires a character class, it improves according to its **class**, not its type.
...
| **Construct** | d10 | HD ×¾ (as cleric) | — | 2 + Int mod per HD³ |
...
| **Ooze** | d10 | HD ×¾ (as cleric) | — | 2 + Int mod per HD³ |
| **Outsider** | d8 | HD (as fighter) | Fort, Ref, Will | 8 + Int mod per HD |
| **Plant** | d8 | HD ×¾ (as cleric) | Fort | 2 + Int mod per HD³ |
| **Undead** | d12 | HD ×½ (as wizard) | Will | 4 + Int mod per HD³ |
| **Vermin** | d8 | HD ×¾ (as cleric) | Fort | 2 + Int mod per HD³ |
...
> **Note**: Repeat the adjustment if the creature moves up more than one size.
...
> **Note**: Repeat the adjustment if the creature moves up more than one size category.
## Templates

Certain creatures are created by adding a **template** to an existing creature. A templated creature can represent:

- A **freak of nature**
- The **individual creation** of a single experimenter
...
> **Note**: It's possible for a certain kind of template to be of either type.

### Reading A Template

A template's description provides a set of instructions for altering an existing creature, known as the **base creature**. The changes that a template might cause to each line of a creature's statistics block are discussed below. 

> **General Rule**: If a template does not cause a change to a certain statistic, that entry is missing from the template description. For clarity, the entry for a statistic or attribute that is not changed is sometimes given as **"Same as the base creature."**
...
**Type Changes:**
- If a template changes the base creature's type, the creature also acquires the **augmented subtype** unless the template description indicates otherwise
...
Most templates do **not** change the number of Hit Dice a monster has, but some do. 

**Template HD Effects:**
- Some templates change the **size of a creature's Hit Dice** (usually by changing the creature type)
- A few templates change **previously acquired Hit Dice**, and continue to change Hit Dice gained with class levels
- Most templates that change Hit Dice change only the creature's **original HD** and leave class Hit Dice unchanged

> **Rule**: If the Hit Dice entry in a template description is missing, Hit Dice and hit points do not change unless the creature's Constitution modifier changes.
...
**Grapple Changes:**
- Changes to a creature's **Strength score** can change a creature's grapple bonus
- Changes to its **size** can also affect grapple bonus

#### Attack and Full Attack
Most templates do **not** change a creature's attack bonus or modes of attack, even when the creature's type changes (the creature's base attack bonus is the same as a creature of the original type). 

**Attack Bonus Adjustments:**
- Any change in **ability scores** may affect attack bonuses
- If **Strength or Dexterity** changes, use the new modifier to determine attack bonuses
- A change in a monster's **size** also changes its attack bonus; see **Table: Changes to Statistics by Size**

#### Damage
Damage changes with **Strength**. 

**Damage Calculations:**
- **Two-handed weapon or single natural weapon**: Adds **1½ times** its Strength bonus to the damage
- **Multiple attacks**: Adds its **Strength bonus** to damage rolls for the primary attack and **½ its Strength bonus** to all secondary attacks

#### Space/Reach
A template may change this entry if it changes the monster's **size**. 

> **Note**: This table does not take into account special situations such as **exceptional reach**.
...
#### Special Qualities
A template may **add or remove** special qualities. The template description gives the details of any special qualities a template provides, including how to determine **saving throw DCs**, if applicable. 

> **Important**: Even if the special qualities entry is missing from a template description, the creature still gains any qualities associated with its **new type**.

#### Base Saves
As with attacks, changing a monster's type does **not** always change its base saving throw bonuses. 

**Save Adjustments:**
- You only need to adjust them for new modifiers for **Constitution, Dexterity, or Wisdom**
- A template may, however, state that a monster has a different **"good" saving throw**

#### Abilities
If a template changes one or more **ability scores**, these changes are noted here.

#### Skills
As with attacks, changing a monster's type does **not** always change its skill points. 

**Skill Adjustments:**
- Most templates don't change the number of Hit Dice a creature has, so you don't need to adjust skills unless:
  - The **key abilities** for those skills have changed
  - The template gives a **bonus on one or more skills**
...
- Treat skills listed in the base creature's description as **class skills**, as well as any new skills provided by the template
...
#### Challenge Rating
Most templates **increase** the creature's Challenge Rating. 

**CR Modifications:**
- A template might provide a **modifier** to be added to the base creature's CR
- It might specify a **range of modifiers** depending on the base creature's original Hit Dice or CR
...
#### Level Adjustment
This entry is a **modifier** to the base creature's level adjustment. 

> **Important**: Any level adjustment is meaningless unless the creature retains a high enough Intelligence (**minimum 3**) to gain class levels after applying the template.
...
- To add more than one template, just apply each template **one at a time**
- **Always apply inherited templates before applying acquired templates**
...
When adding class levels to a creature, you should give it typical ability scores appropriate for that class:

- **Most creatures**: Use the **standard array** of ability scores: **11, 11, 11, 10, 10, 10**, adjusted by racial modifiers
- **PC classes**: Use the **elite array** before racial adjustments: **15, 14, 13, 12, 10, 8**
- **NPC classes**: Use the **nonelite array**: **13, 12, 11, 10, 9, 8**
...
**Examples of Associated Classes:**
...
If you add a class level that **doesn't directly play to a creature's strength** the class level is considered **nonassociated**, and things get a little more complicated. 
...
> **Important**: Levels in **NPC classes** are always treated as nonassociated.

## Adding Hit Dice

When you improve a monster by adding Hit Dice, use **Table: Improved Monster CR Increase** to determine the effect on the creature's CR. 

**Important Considerations:**
- Many monsters that advance by adding Hit Dice also **increase in size**
- Do **not** stack this CR increase with any increase from class levels
- Once you've **doubled a creature's CR**, you should closely watch any additional increases in its abilities
...
> **Note**: Do not apply this increase if you advance a monster by class levels. (Monsters advanced by class levels are assumed to use the elite array.)
### Increasing Size
...
> **⚠️ Warning**: Monsters that benefit from a smaller size may actually **lose effectiveness** because of a size increase. Monsters that don't benefit from size increases don't advance in that manner for this reason.

## Adding Special Abilities

You can add any sort of **spell-like**, **supernatural**, or **extraordinary** ability to a creature. As with a class level, you should determine how much, or how little, this ability adds to the creature's existing repertoire. 

### CR Modifications for Special Abilities

- **Significant abilities**: Increase CR by **+2**
- **Minor abilities**: Increase CR by **+1**
- **Trivial abilities**: May not increase CR at all

> **Note**: A suite of abilities that work together should be treated as a **single modifier** for this purpose. If the special abilities a monster gains are not tied to a class or Hit Die increase, this CR increase stacks.

### Defining Significant vs. Minor Abilities

- **Significant special attack**: One that stands a good chance of **incapacitating or crippling** a character in one round
- **Significant special quality**: One that **seriously diminishes** the monster's vulnerability to common attacks

> **Important**: Do not add this factor twice if a monster has both special attacks and special qualities.

> **Scaling**: Make sure to "scale" your evaluation of these abilities by the monster's **current CR**.

---

> **📖 Related Sections**: [Monster Types](creature-types/core-types.md), [Monster Subtypes](subtypes/), [Combat](../combat/), [Special Abilities](../abilities/)
