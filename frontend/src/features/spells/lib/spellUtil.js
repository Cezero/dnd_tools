...
export const getClassDisplay = (classes, spellLevel) => {
    if (!classes || classes.length === 0) return '';

    const formattedClasses = classes.map(cls => {
        const classItem = LookupService.getById('classes', cls.class_id);
        if (classItem) {
...
export const getClassLevelAbbr = (classLevels) => {
    ...
    classLevels.forEach(cl => {
        const classItem = LookupService.getById('classes', cl.class_id);
        if (classItem) {
