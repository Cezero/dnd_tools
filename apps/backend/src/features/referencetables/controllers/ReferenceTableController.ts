...
export async function GetReferenceTableData(identifier: string | number) {
    let table;
    ...
    cells.forEach(cell => {
        const rowIndex = rowMap[cell.rowId].rowIndex;
        const colIndex = colMap[cell.columnId].columnIndex;
...
export async function Resolve(identifiers: string[]): Promise<any[]> {
    const results = [];
