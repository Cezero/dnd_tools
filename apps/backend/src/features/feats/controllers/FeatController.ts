...
export const GetFeats = async (req: FeatRequest, res: Response): Promise<void> => {
    try {
        ...
        if (req.query.feat_name) {
            where.name = { contains: req.query.feat_name, mode: 'insensitive' };
        }
        if (req.query.feat_type) {
            where.typeId = parseInt(req.query.feat_type);
        }
        if (req.query.feat_description) {
            where.description = { contains: req.query.feat_description, mode: 'insensitive' };
        }
        if (req.query.feat_benefit) {
            where.benefit = { contains: req.query.feat_benefit, mode: 'insensitive' };
        }
        if (req.query.feat_normal) {
            where.normalEffect = { contains: req.query.feat_normal, mode: 'insensitive' };
        }
        if (req.query.feat_special) {
            where.specialEffect = { contains: req.query.feat_special, mode: 'insensitive' };
        }
        if (req.query.feat_prereq) {
            where.prerequisites = { contains: req.query.feat_prereq, mode: 'insensitive' };
        }
