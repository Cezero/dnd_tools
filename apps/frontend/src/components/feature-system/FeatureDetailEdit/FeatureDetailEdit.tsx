...
import type { FeatureWithRelations, Feature, FeatureEntity } from '@shared/schema';
...
import type { EntityTypeConfig, FeatureDetailEditProps } from './types';
...
export function FeatureDetailEdit({
...
}: FeatureDetailEditProps) {
    const cacheFunctions = useCacheFunctions();
    const editionIdRef = useRef<number | null>(null);
    const featureKeyRef = useRef<string>('');

    // Determine editionId: use provided, or get from cache based on feature source
    ...
    const editionId = useMemo(() => {
        ...
        // Create a stable key from feature properties
        const firstClassId = feature?.classes && feature.classes.length > 0 ? feature.classes[0].classId : '';
        const firstRaceId = feature?.races && feature.races.length > 0 ? feature.races[0].raceId : '';
        const featureKey = feature
            ? `${feature.sourceType}-${firstClassId}-${firstRaceId}-${feature.featId ?? ''}-${feature.editionId ?? ''}`
            : '';

        // Only recalculate if the feature key actually changed
        if (featureKey === featureKeyRef.current && editionIdRef.current !== null) {
            return editionIdRef.current;
        }

        featureKeyRef.current = featureKey;

        // Try to get editionId from cache based on feature source
        let calculatedEditionId: number | null = null;
        if (feature) {
            if (feature.sourceType === FeatureSourceType.Class && feature.classes && feature.classes.length > 0) {
            ...
            } else if (feature.sourceType === FeatureSourceType.Race && feature.races && feature.races.length > 0) {
            ...
            } else if (feature.sourceType === FeatureSourceType.Feat && feature.featId) {
                const featData = cacheFunctions.getFeatSummaryById(feature.featId);
                calculatedEditionId = featData?.editionId ?? null;
            } else if (feature.sourceType === FeatureSourceType.Edition && feature.editionId) {
                calculatedEditionId = feature.editionId;
            }
        }
    ...
    const {
        ...
        schema
    } = useFeatureForm(feature, preSelectedFeature);

    const {
        groupingState,
        setGroupingState,
        updateEntityGrouping
    } = useGroupingState(feature);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Check for malformed data - every FeatureWithRelations should have an id (0 is valid for new features)
        // Note: featureId in formData is for linking to feature templates, but the feature itself uses id
        if (formData.id === null || formData.id === undefined) {
            console.error('Malformed FeatureWithRelations: missing featureId', {
                formData,
                feature,
                preSelectedFeature
            });
        ...
        try {
            ...
            onSave(formData as FeatureWithRelations);
            onClose();
        } catch (error) {
                    ...
                    <div className="w-full max-w-4xl max-h-[90vh] transform rounded-2xl bg-white dark:bg-gray-800 flex flex-col shadow-xl transition-all">
                        ...
                        <div className="p-6 border-b border-gray-200 dark:border-gray-600 flex-shrink-0">
                            <Dialog.Title className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                                {feature ? 'Edit' : 'Add'} {preSelectedFeature?.name || feature?.name || 'Feature'} Feature
                            </Dialog.Title>
                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                Level {Number(formData.level) || 1} - {preSelectedFeature?.name || feature?.name || 'Feature'}
                            </p>
                            {(() => {
                                ...
                                return (
                                    <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-700/20 rounded-md">
                                        <p className="text-xs text-gray-600 dark:text-gray-400">
                                            <strong>Multi-Section Feature Feature:</strong> Select which components this feature provides. A single feature can include entities, special effects, and other components simultaneously.
                                        </p>
                                    </div>
                                );
                            })()}
                            {/* Feature Prerequisites Display - use formatting system (Phase 6) */}
                            {((feature as Feature)?.prerequisites?.length > 0 || (preSelectedFeature as Feature)?.prerequisites?.length > 0) && (() => {
                                // Format prerequisites using the display strategy system
                                const baseFeature = (feature || preSelectedFeature) as Feature;
                                // FeatureWithRelations is now the unified Feature model
                                const featureForFormatting: FeatureWithRelations = feature || {
                                    ...baseFeature,
                                    id: baseFeature?.id || 0,
                                    sourceType: 0,
                                ...
                                const displayResult = strategy.format(featureForFormatting);
