...
import { SPELL_DESCRIPTOR_LIST, SPELL_SCHOOL_LIST, SPELL_COMPONENT_LIST, SPELL_RANGE_LIST, SPELL_RANGE_MAP, SPELL_SUBSCHOOL_LIST_BY_SCHOOL_ID } from '@shared/static-data/SpellData';
import { SOURCE_BOOK_LIST } from '@shared/static-data/SourceData';
import { CLASS_LIST, CLASS_MAP } from '@shared/static-data/ClassData';
...
export function SpellEdit(): React.JSX.Element {
    ...
    const HandleChange = (e) => {
        ...
        if (name === 'range_id') {
            const selectedRange = SPELL_RANGE_MAP[parseInt(value)];
            setSpell(prevSpell => ({
                ...prevSpell,
                range_id: parseInt(value),
                range_str: selectedRange ? selectedRange.range_name : ''
            }));
    ...
    const HandleClassLevelChange = (index, field, value) => {
        setSpell(prevSpell => {
            if (field === 'spell_level' && parseInt(value) === -1) {
                ...
                return {
                    ...prevSpell,
                    class_levels: prevSpell.class_levels.toSpliced(index, 1)
                };
            } else {
                return {
                    ...prevSpell,
    ...
    const HandleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                effect_desc: spell.effect_desc || null,
                target_desc: spell.target_desc || null
            };
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-2 auto-rows-auto">
                    <div className="md:col-span-2">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="range_str" className="block text-lg w-48 font-medium">Range (Text):</label>
                            <input type="text" id="range_str" name="range_str" value={spell.range_str || ''} readOnly className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600 bg-gray-100" />
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="range_id" className="block text-lg w-48 font-medium">Range:</label>
                            <select id="range_id" name="range_id" value={spell.range_id || ''} onChange={HandleChange} className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="range_value" className="block text-lg w-48 font-medium">Range Value:</label>
                            <input type="text" id="range_value" name="range_value" value={spell.range_value || ''} onChange={HandleChange} className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="effect_desc" className="block text-lg w-48 font-medium">Effect:</label>
                            <input type="text" id="effect_desc" name="effect_desc" value={spell.effect_desc || ''} onChange={HandleChange} className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="area_desc" className="block text-lg w-48 font-medium">Area:</label>
                            <input type="text" id="area_desc" name="area_desc" value={spell.area_desc || ''} onChange={HandleChange} className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="target_desc" className="block text-lg w-48 font-medium">Target:</label>
                            <input type="text" id="target_desc" name="target_desc" value={spell.target_desc || ''} onChange={HandleChange} className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="duration_desc" className="block text-lg w-48 font-medium">Duration:</label>
                            <input type="text" id="duration_desc" name="duration_desc" value={spell.duration_desc || ''} onChange={HandleChange} className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="save_desc" className="block text-lg w-48 font-medium">Saving Throw:</label>
                            <input type="text" id="save_desc" name="save_desc" value={spell.save_desc || ''} onChange={HandleChange} className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                            <label htmlFor="sr_desc" className="block text-lg w-48 font-medium">Spell Resistance:</label>
                            <input type="text" id="sr_desc" name="sr_desc" value={spell.sr_desc || ''} onChange={HandleChange} className="mt-1 block w-full p-1 border rounded dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                    ...
                    <div className="flex items-center gap-2">
                        {spell.class_levels.map((cl, index) => (
                            <div key={index} className="flex items-center gap-1">
                                <span className="block">{CLASS_MAP[cl.class_id]?.name || 'Unknown Class'}:</span>
                                <Listbox
                        ...
                        >
                            ...
                            {CLASS_LIST
                                .filter(cls => {
                                    const classIdsInUse = new Set(spell.class_levels.map(c => c.class_id));
