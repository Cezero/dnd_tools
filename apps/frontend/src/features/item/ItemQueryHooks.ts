 ...
 */
const createItemByIdQueryFn = (originalQueryFn: (params?: unknown) => Promise<ItemWithDetails>) => {
    return async (contextOrParams: QueryFunctionContext | { pathParams?: { id?: number } } | undefined): Promise<ItemWithDetails> => {
 ...
 * It first checks if the item exists in the ['items', 'list'] cache, and only makes an API
 * call if the item is not found in cache.
 */
 ...
 * 2. If item found in cache, return it immediately
 * 3. Cache item under ['items', 'item', id] for consistency
 ...
 */
const itemByIdFetch = async (params?: unknown, options?: { staleTime?: number }, queryClient?: QueryClient) => {
    if (!queryClient) {
    ...
    return queryClient.fetchQuery({
        queryKey: itemByIdBaseConfig.queryKeyBuilder(params),
        queryFn: () => itemByIdQueryFn(params),
...
export const ItemQueryHooks = {
    ...
    // Imperative methods
    getAllItems: (params?: unknown) => itemsConfig.fetch(params),
    ...
    updateItem: (itemId: number, data: unknown, queryClient?: QueryClient) => updateItemConfig.mutate({
        requestData: data,
    ...
    getAllItemsQueryFn: itemsConfig.queryFn,
    getItemByIdQueryFn: itemByIdConfig.queryFn,
    getAllItemsQueryKey: (params?: unknown) => itemsConfig.queryKeyBuilder(),
