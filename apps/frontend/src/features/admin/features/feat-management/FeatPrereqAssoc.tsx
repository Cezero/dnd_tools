...
import { FetchFeatPrereqs } from '@/features/admin/features/feat-management/FeatPrereqService';
import { FEAT_PREREQUISITE_TYPES, FEAT_PREREQUISITE_TYPE_LIST } from '@shared/static-data';

// Type for feat prerequisite items
type FeatPrereqItem = {
    prereq_id: number;
    feat_id: number;
    prereq_type: number;
    prereq_type_id: string;
 ...
 */
export function FeatPrereqAssoc({ isOpen, onClose, onSave, initialSelectedPrereqIds = [], featId }) {
    const navigate = useNavigate();
    const [currentSelectedPrereqIds, setCurrentSelectedPrereqIds] = useState(initialSelectedPrereqIds);
    const [availablePrereqs, setAvailablePrereqs] = useState<FeatPrereqItem[]>([]); // To store all prerequisites fetched by GenericList
    ...
    const filterOptions = useMemo(() => ({
        prereq_id: { component: TextInput, props: { placeholder: 'Filter by ID...' } },
        prereq_type: { component: TextInput, props: { placeholder: 'Filter by type...' } },
    }), []);
    ...
    const renderPrereqCell = useCallback((item: FeatPrereqItem, columnId: string) => {
        if (columnId === 'prereq_type') {
            return FEAT_PREREQUISITE_TYPES[item[columnId]]?.name || item[columnId];
        }
        return String(item[columnId as keyof FeatPrereqItem] || '');
    }, []);
