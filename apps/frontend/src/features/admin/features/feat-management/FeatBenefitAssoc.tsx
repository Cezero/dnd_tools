...
import { FetchFeatBenefits } from '@/features/admin/features/feat-management/FeatBenefitService';
import { FEAT_BENEFIT_TYPES, FEAT_BENEFIT_TYPE_LIST } from '@shared/static-data';

// Type for feat benefit items
type FeatBenefitItem = {
    benefit_id: number;
    feat_id: number;
    benefit_type: number;
...
export function FeatBenefitAssoc({ isOpen, onClose, onSave, initialSelectedBenefitIds = [], featId }) {
    ...
    const [availableBenefits, setAvailableBenefits] = useState<FeatBenefitItem[]>([]); // To store all benefits fetched by GenericList
    ...
    const filterOptions = useMemo(() => ({
        benefit_id: { component: TextInput, props: { placeholder: 'Filter by ID...' } },
        benefit_type: { component: TextInput, props: { placeholder: 'Filter by type...' } },
    }), []);
    ...
    const renderBenefitCell = useCallback((item: FeatBenefitItem, columnId: string) => {
        if (columnId === 'benefit_type') {
            return FEAT_BENEFIT_TYPES[item[columnId]]?.name || item[columnId];
        }
        return String(item[columnId as keyof FeatBenefitItem] || '');
    }, []);
