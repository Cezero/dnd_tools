...
import { COLUMN_DEFINITIONS, DEFAULT_COLUMNS, ClassFilterOptions } from '@/features/admin/features/classMgmt/ClassConfig';
import { FetchClasses, DeleteClass } from '@/features/admin/features/classMgmt/ClassService';
import { UseAuth } from '@/components/auth/authProvider';
import { RPG_DICE, EDITION_MAP } from '@shared/static-data';
import type { ClassItem } from '@shared/prisma-client';
...
export function ClassList(): React.JSX.Element {
    ...
    const RenderCell = (item: ClassItem, columnId: string): React.ReactNode => {
        const column = COLUMN_DEFINITIONS[columnId];
        if (!column) return null;

        let cellContent: React.ReactNode = String(item[columnId as keyof ClassItem] || '');
