...
def test_get_spell_by_id(api: APIClient):
    ...
    assert "spell_area" in spell
    assert "spell_description" in spell
    assert "spell_duration" in spell
    assert "spell_name" in spell
    assert "spell_range" in spell
    assert "spell_save" in spell
    assert "spell_resistance" in spell
...
def test_get_spells_with_search(api: APIClient):
    ...
    assert "results" in data
    for spell in data["results"]:
        assert "fire" in spell["spell_name"].lower() or "fire" in spell.get("spell_summary", "").lower()
...
def test_get_spells_with_sorting(api: APIClient):
    ...
    data = response.json()
    assert "results" in data
    names = [spell["spell_name"] for spell in data["results"]]
